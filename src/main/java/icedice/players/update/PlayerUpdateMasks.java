...
package icedice.players.update;


import icedice.Engine;
import icedice.util.Misc;
import icedice.util.Stream;
import icedice.util.Vars;
import icedice.io.packets.Equipment;
...
public class PlayerUpdateMasks {
    ...
    private void calculateCombat(Player p) {
        ...
        int summon = p.getLevelForXP(23);

        p.combatLevel = (int) ((defence + hp + Math.floor(prayer / 2) + Math.ceil((summon / 2) + 1)) * 0.25) + 0;
