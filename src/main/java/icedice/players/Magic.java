...
public class Magic {
    ...
    public static void teleTo(Player p, int x, int y) {
        p.teleportTo(x, y, 0, 4, 4, 8939, 8941, 1678, 0, 1679, 0);
    }

    public static void teleTo(Player p, int x, int y, int h) {
        p.teleportTo(x, y, h, 4, 4, 8939, 8941, 1678, 0, 1679, 0);
    }
    ...
    public boolean ancientMagicAB(Player p, int buttonID) {
        ...
        switch (buttonID) {
            ...
            case 22: // Party Teleport
                if (p.magicDelay <= 0) {
                    if (magicLvl(p) >= ancientLvlReq[buttonID]) {
                        if (hasReq(p, law, 2) && hasReq(p, blood, 2)) {
                            ancientTeleTo(p, 2197, 3252);
            ...
            case 21: // Mithril Teleport
                if (p.magicDelay <= 0) {
                    if (magicLvl(p) >= ancientLvlReq[buttonID]) {
                        if (hasReq(p, law, 2) && hasReq(p, soul, 2)) {
                            ancientTeleTo(p, 2999, 3383);
            ...
            case 20: // Shops Teleport
                if (p.magicDelay <= 0) {
                    if (magicLvl(p) >= ancientLvlReq[buttonID]) {
                        if (hasReq(p, law, 2) && hasReq(p, air, 2)
                                && hasReq(p, fire, 3)) {
                            ancientTeleTo(p, Config.hShopsX, Config.hShopsY);
            ...
            case 17: // Bounty Teleport
                if (p.magicDelay <= 0) {
                    if (magicLvl(p) >= ancientLvlReq[buttonID]) {
                        if (hasReq(p, law, 2) && hasReq(p, soul, 1)) {
                            ancientTeleTo(p, 3180, 3685);
            ...
            case 16: // Chill Teleport -- FIX
                if (p.magicDelay <= 0) {
                    if (magicLvl(p) >= ancientLvlReq[buttonID]) {
                        if (hasReq(p, law, 2) && hasReq(p, soul, 1)) {
                            ancientTeleTo(p, 2464, 4845);
                            deleteItems(p, law, 2, soul, 1);
                            magicXp(p, ancientSpellXp[buttonID]);
                        } else {
                            sendMsg(p,
    ...
    public boolean modernMagicAB(Player p, int buttonID) {
        ...
        switch (buttonID) {
            ...
            case 58: // Charge
                if (p.magicDelay <= 0) {
                    if (magicLvl(p) >= modernLvlReq[buttonID]) {
                        if (hasReq(p, 565, 3) && hasReq(p, fire, 3) && hasReq(p, air, 3)) {
                            deleteItems(p, air, 3, fire, 3, 565, 3);
                            sendMsg(p, "You feel a sudden urge run through your veins.");
                            anim(p, 811);
                            magicXp(p, modernSpellXp[buttonID]);
                        } else {
    ...
    public static void magicAop(Player p, int packetId, int packetSize) {
        ...
        } else if (!Engine.wildernessArea(p.absX, p.absY) && p.AtDuel() == false && p.AtClanField() == false) {
            p.frames.sendMessage(p, "You are not in the wilderness!");
                            ...
                            if (p.magicDelay <= 0) {
                                if (magicLvl(p) >= ancientLvlReq[p.clickId]) {
                                    if (hasReq(p, death, 4) && hasReq(p, fire, 4)
                                        ...
                                        magicXp(p, hitdmg * 2 + ancientSpellXp[p.clickId]);
                                        Engine.poison.startPoison(p2, 4);
