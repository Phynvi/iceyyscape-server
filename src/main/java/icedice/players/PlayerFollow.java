...
public class PlayerFollow {
    ...
    public void followPlayer(Player p) {
        ...
        if ((p2.absX - p.absX) <= (n * (-1)) && (p2.absX - p.absX) >= (n - 1)) {
            if (n < 0) {
                p.reqWalkQueue(p2.absX, p2.absY + 1);
            }
            if (n > 0) {
                p.reqWalkQueue(p2.absX, p2.absY - 1);
            }
        } else if ((p2.absX - p.absX) > 0) {
            p.reqWalkQueue(p2.absX - 1, p2.absY);
        } else if ((p2.absX - p.absX) < 0) {
            p.reqWalkQueue(p2.absX + 1, p2.absY);
        }
        ...
        if (p2.absX - p.absX > 12 || p.absX - p2.absX > 12 || p2.absY - p.absY > 12 || p.absY - p2.absY > 12) {
            p.reqWalkQueue(p.absX, p.absY); //Reset walking if the other player is further then 12 squares (To prevent the following if p2 teleports away)
        }
