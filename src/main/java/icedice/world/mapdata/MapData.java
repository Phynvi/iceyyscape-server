...
public class MapData {
     ...
     */
    public MapList[] mapLists = new MapList[1000];
    ...
    public boolean needsHome = false;
    ...
    public void loadMapAreaData() {
        ...
        for (int i = 0; i < 16000; i++) {
            try {
                File file = new File(Config.getPropertiesValue("MAP_DATA_PATH") + i + ".txt");

                if (file.exists()) {
                    MapList list = mapLists[curId++] = new MapList();
                    BufferedReader in = new BufferedReader(
                            new FileReader(Config.getPropertiesValue("MAP_DATA_PATH") + i + ".txt"));
                    String str;
                    int regionId = 0;
